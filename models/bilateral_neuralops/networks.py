...
############################################################################################################
class SMBilateralOperator(nn.Module):
	"""
	This operator learns a bilateral grid from down-sampled input image, 
	when forward, it slices the learned bilateral grid using full-res input image
	to get a full-res output image. 
...
class BilateralRenderer(nn.Module):
	def __init__(self, n_in, n_out, lowres, luma_bins, spatial_bins, channel_multiplier, guide_pts, norm, iteratively_upsample):
		super(BilateralRenderer,self).__init__()
		self.in_nc = n_in
		self.out_nc = n_out
		self.ex_block = BilateralOperator(n_in, n_out, lowres, luma_bins, spatial_bins, channel_multiplier, guide_pts, norm, iteratively_upsample)
		self.bc_block = BilateralOperator(n_in, n_out, lowres, luma_bins, spatial_bins, channel_multiplier, guide_pts, norm, iteratively_upsample)
		self.vb_block = BilateralOperator(n_in, n_out, lowres, luma_bins, spatial_bins, channel_multiplier, guide_pts, norm, iteratively_upsample)
...
############################################################################################################
class BilateralEWVRenderer(nn.Module):
	def __init__(self, n_in, n_out, lowres, luma_bins, spatial_bins, channel_multiplier, guide_pts, norm, iteratively_upsample):
		super(BilateralEWVRenderer,self).__init__()
		self.in_nc = n_in
		self.out_nc = n_out
		self.ex_block = BilateralOperator(n_in, n_out, lowres, luma_bins, spatial_bins, channel_multiplier, guide_pts, norm, iteratively_upsample)
		self.wb_block = BilateralOperator(n_in, n_out, lowres, luma_bins, spatial_bins, channel_multiplier, guide_pts, norm, iteratively_upsample)
		self.vb_block = BilateralOperator(n_in, n_out, lowres, luma_bins, spatial_bins, channel_multiplier, guide_pts, norm, iteratively_upsample)
...
############################################################################################################
class ColorV2BilateralRenderer(nn.Module):
	def __init__(self,
			n_in=3,
			n_out=3,
			lowres=[256, 256],
		...
		super(ColorV2BilateralRenderer, self).__init__()
		self.n_in = n_in
		self.n_out = n_out
...
class SMColorBilateralRenderer(nn.Module):
	def __init__(self,
		...
		super(SMColorBilateralRenderer, self).__init__()
		self.n_in = n_in
		self.n_out = n_out
		...
		self.wb_block = SMBilateralOperator(n_in, n_out, lowres, luma_bins, spatial_bins, channel_multiplier, guide_pts, norm, iteratively_upsample)
...
############################################################################################################
class SMBilateralRenderer(nn.Module):
	def __init__(self,
			n_in=3,
			n_out=3,
			lowres=[256, 256],
...
class BilateralNeurOP(nn.Module):
	def __init__(self,
		...
		renderer = BilateralRenderer(n_in, n_out, lowres, luma_bins, spatial_bins, channel_multiplier, guide_pts, norm, iteratively_upsample) # TODO: pass bilateral params here
...
############################################################################################################
class BilateralEWVNeurOP(nn.Module):
	def __init__(self,
				 # neural op params
				 n_in = 3,
				 n_out = 3,
		...
		super(BilateralEWVNeurOP,self).__init__()

		self.fea_dim = encode_nf * 3
		self.image_encoder = Encoder(n_in,encode_nf)
		renderer = BilateralEWVRenderer(n_in, n_out, lowres, luma_bins, spatial_bins, channel_multiplier, guide_pts, norm, iteratively_upsample)
		if load_path is not None: 
			renderer.load_state_dict(torch.load(load_path))
...
############################################################################################################
class TorchBilateralNeurOP(nn.Module):
	def __init__(self,
				 # neural op params
				 n_in = 3,
				 n_out = 3,
		...
		super(TorchBilateralNeurOP,self).__init__()

		self.fea_dim = encode_nf * 3
		self.image_encoder = Encoder(n_in,encode_nf)
		renderer = TorchSimpleBilateralRenderer(n_in, n_out) # TODO: pass bilateral params here
		if load_path is not None: 
			renderer.load_state_dict(torch.load(load_path))
...
############################################################################################################
class SMColorBilateralNeurOP(nn.Module):
	def __init__(self,
				 # neural op params
				 n_in = 3,
				 n_out = 3,
		...
		super(SMColorBilateralNeurOP,self).__init__()

		self.fea_dim = encode_nf * 3
		self.image_encoder = Encoder(n_in,encode_nf)
		renderer = SMColorBilateralRenderer(n_in, n_out, lowres, luma_bins, spatial_bins, channel_multiplier, guide_pts, norm, iteratively_upsample) # TODO: pass bilateral params here
		if load_path is not None: 
			renderer.load_state_dict(torch.load(load_path))
...
############################################################################################################
class SMBilateralNeurOP(nn.Module):
	def __init__(self,
				 # neural op params
				 n_in = 3,
				 n_out = 3,
...
############################################################################################################
class ColorV2BilateralNeurOP(nn.Module):
	def __init__(self,
				 # neural op params
				 n_in = 3,
				 n_out = 3,
		...
		super(ColorV2BilateralNeurOP,self).__init__()

		self.fea_dim = encode_nf * 3
		self.image_encoder = Encoder(n_in,encode_nf)
		renderer = ColorV2BilateralRenderer(n_in, n_out, lowres, luma_bins, spatial_bins, channel_multiplier, guide_pts, norm, iteratively_upsample) # TODO: pass bilateral params here
		if load_path is not None: 
			renderer.load_state_dict(torch.load(load_path))
