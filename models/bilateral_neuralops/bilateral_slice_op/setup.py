...
def check_env_flag(name, default=''):
    print("[CHECK_ENV_FLAG]: ", name, os.getenv(name, default).upper())
    return os.getenv(name, default).upper() in ['ON', '1', 'YES', 'TRUE', 'Y']


def check_negative_env_flag(name, default=''):
    print("[CHECK_NEG_ENV_FLAG]: ", name, os.getenv(name, default).upper())
...
setup(name='bilateral_slice_op',
      ...
      extra_compile_args = ["-O0"] # disable optimization 
      )
