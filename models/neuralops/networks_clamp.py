...
class NeurOP(nn.Module):
    def __init__(self, in_nc = 3, out_nc = 3, base_nf = 64, encode_nf = 32 , load_path = None, return_vals = False):
...
#  dt(torch.clamp(nop(img,-0.9)[0],0,1), torch.clamp(img[0],0,1))
#  from utils import display_tensors as dt